...
//
//  BUILD WITH CUSTOM ICON (MinGW-w64):
//    windres AlienwarePowerScript.rc -O coff -o AlienwarePowerScript.res
//    g++ -O2 -municode -mwindows -o AlienwarePowerScript.exe AlienwarePowerScript_CustomTrayIcon.cpp AlienwarePowerScript.res -luser32 -lgdi32 -lshell32
//
//  BUILD WITH CUSTOM ICON (MSVC "x64 Native Tools" prompt):
//    rc /fo AlienwarePowerScript.res AlienwarePowerScript.rc
//    cl /O2 /EHsc /DUNICODE /D_UNICODE AlienwarePowerScript_CustomTrayIcon.cpp AlienwarePowerScript.res /link user32.lib gdi32.lib shell32.lib /SUBSYSTEM:WINDOWS
//
//  NOTES:
...
static const wchar_t* APP_NAME  = L"Alienware Power Script";
...
void CreateTrayIcon(HWND hwnd)
{
    ...
    // Load the icon embedded in the EXE by AlienwarePowerScript.rc.
    // LoadImage gives us an icon handle that we own and can DestroyIcon later.
    ...
    // Fallback so the tray icon still exists if the resource was not linked.
    if (!g_hIcon)
        g_hIcon = CopyIcon(LoadIconW(nullptr, IDI_APPLICATION));
    ...
    // NIF_SHOWTIP is important when using NOTIFYICON_VERSION_4; without it
    // Windows may suppress the normal hover tooltip.
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TRAYICON:
        if (LOWORD(lParam) == WM_RBUTTONUP)
        {
            ...
            // Friendly app title at the top of the tray menu.
            AppendMenuW(hMenu, MF_STRING | MF_DISABLED, 0, APP_NAME);
